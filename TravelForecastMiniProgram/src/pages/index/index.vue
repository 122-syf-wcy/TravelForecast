    ...
    <view class="hero" :style="{ height: heroH + 'px' }">
      ...
      <view class="navbar" :style="{ paddingTop: stBar + 'px' }">
        ...
        <view class="nav-search" @tap="onSearch">
          <view class="s-icon">
            <image class="s-icon-img" :src="commonIcons.search" mode="aspectFit" />
          </view>
    ...
    <view class="menu-bar">
      <view class="mi" v-for="(m, i) in menuList" :key="i" @tap="onMenu(m)">
        <view class="mi-icon" :style="{ background: m.bg }">
          <image class="mi-icon-img" :src="m.icon" mode="aspectFit" />
        </view>
      ...
      <view class="card-col">
        <view class="card-sm card-warm" @tap="onSeason">
          ...
          <view class="cs-icon snow-icon">
            <image class="cs-icon-img" :src="quickCardIcons.season" mode="aspectFit" />
          </view>
        </view>
        <view class="card-sm card-red" @tap="onRedStudy">
          ...
          <view class="cs-icon red-icon">
            <image class="cs-icon-img" :src="quickCardIcons.redStudy" mode="aspectFit" />
          </view>
        </view>
      </view>
    </view>
...
import { COMMON_ICONS, HOME_CATEGORY_LIST, HOME_MENU_LIST, QUICK_CARD_ICONS } from '@/utils/icon-catalog'
...
const quickCardIcons = QUICK_CARD_ICONS

const getSeasonTemp = () => {
  const m = new Date().getMonth() + 1
  const d = new Date().getDate()
  const base = { 1: 4, 2: 6, 3: 11, 4: 16, 5: 19, 6: 22, 7: 24, 8: 23, 9: 20, 10: 15, 11: 10, 12: 5 }
  const t = (base[m] || 19) + Math.round((d % 5) - 2)
  return `${t}°C`
}
...
const menuList = HOME_MENU_LIST

const catList = HOME_CATEGORY_LIST
...
const loadHomeData = async () => {
  try {
    ...
    if (Array.isArray(spots) && spots.length > 0) {
      const FALLBACK_IMG = '/static/default-spot.svg'
    ...
    // 保留默认展示，避免影响首屏
    console.warn('[home] load failed, keep defaults:', err && err.message)
  }
}
...
const onBannerTap = (b) => {
  if (b.link && typeof b.link === 'string') {
    if (b.link.startsWith('/pages/')) {
      ...
      uni.navigateTo({ url: `/pages/webview/index?url=${encodeURIComponent(b.link)}` })
    }
  }
}
...
.s-icon-img { width: 14px; height: 14px; display: block; }
.s-text {
  font-size: 13px;
...
.mi-icon-img { width: 24px; height: 24px; position: relative; z-index: 2; }
.mi-name {
  font-size: 11px;
  color: #333;
  font-weight: 600;
...
.cs-icon-img { width: 20px; height: 20px; display: block; }
...
.cat-icon-img { width: 20px; height: 20px; position: relative; z-index: 2; }
.cat-name {
  font-size: 11px;
  color: #444;
  font-weight: 500;
