    ...
    <scroll-view scroll-x class="tabs" :show-scrollbar="false">
      ...
      >
        <view class="tab-icon" :style="{ background: c.bg }">
          <image class="tab-icon-img" :src="c.icon" mode="aspectFit" />
        </view>
    ...
    <view class="list" v-else>
      <view class="item" v-for="(item, i) in items" :key="i" @tap="openItem(item)">
        <view class="item-cover">
          ...
          <view v-else class="item-img-ph" :style="{ background: item.coverBg }">
            <image class="item-img-icon" :src="item.coverIcon" mode="aspectFit" />
          </view>
        </view>
...
import { decorateServiceCategory, HOME_CATEGORY_LIST, resolveServiceItemVisual } from '@/utils/icon-catalog'
...
const fallbackCategories = HOME_CATEGORY_LIST

const loadCategories = async () => {
  try {
    const res = await fetchServiceCategories()
    const source = Array.isArray(res) && res.length > 0 ? res : fallbackCategories
    categories.value = source.map(decorateServiceCategory)
  } catch (e) {
...
const normalizeItem = (item) => {
  ...
  const visual = resolveServiceItemVisual(item)
  return {
    ...item,
    imageUrl,
    coverBg: visual.coverBg,
    coverIcon: visual.coverIcon
  }
}
...
.tab-icon-img { width: 18px; height: 18px; display: block; }
...
.item-img-icon { width: 26px; height: 26px; display: block; }
