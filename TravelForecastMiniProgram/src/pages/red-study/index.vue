  ...
  <view class="page">
    ...
    <view class="quiz-sec" v-if="curTab === 1">
      <view class="quiz-progress">
        <text class="qp-t">第 {{ quizIndex + 1 }} / {{ quizList.length || 0 }} 题</text>
        <view class="qp-bar">
          <view class="qp-fill" :style="{ width: (quizList.length ? ((quizIndex + 1) / quizList.length * 100) : 0) + '%' }" />
        </view>
      </view>
      ...
      <view class="quiz-card" v-if="currentQuiz">
        ...
        <view class="qc-result" v-if="quizAnswered">
          <view class="qr-badge" :class="quizCorrect ? 'qr-ok' : 'qr-err'">
            <image class="qr-icon" :src="quizCorrect ? commonIcons.checkCircle : commonIcons.xCircle" mode="aspectFit" />
    ...
    <view class="passport-sec" v-if="curTab === 2">
      <view class="pp-header">
        <view class="pp-avatar">
          <image class="pp-av-icon" :src="commonIcons.passport" mode="aspectFit" />
        </view>
        <view class="pp-info">
...
import { COMMON_ICONS, decorateProfileBadge } from '@/utils/icon-catalog'
...
const bases = ref([])
const baseFallback = [
  { id: 'red-base-1', title: '三线建设博物馆', desc: '了解三线建设历史与工业记忆，适合亲子与研学团队打卡。', loc: '六盘水·钟山区', img: '/static/default-spot.svg' },
  { id: 'red-base-2', title: '盘州会议会址', desc: '结合党史教育与实地参访，适合作为红色主题研学线路起点。', loc: '六盘水·盘州市', img: '/static/default-spot.svg' },
  { id: 'red-base-3', title: '乌蒙山红军长征纪念点', desc: '重温长征故事，配合徒步讲解可形成半日研学体验。', loc: '六盘水·水城区', img: '/static/default-spot.svg' }
]
...
const quizFallback = [
  {
    id: 'quiz-1',
    spot: '三线建设博物馆',
    question: '“三线建设”主要发生在我国哪个历史时期？',
    options: ['改革开放初期', '20世纪60至70年代', '新中国成立初期', '21世纪初'],
    answer: 1,
    explanation: '三线建设主要集中在20世纪60至70年代，是我国重要的战略建设工程。'
  },
  {
    id: 'quiz-2',
    spot: '盘州会议会址',
    question: '红色研学最核心的学习价值之一是什么？',
    options: ['单纯拍照打卡', '了解历史、传承精神', '购买纪念品', '完成签到任务'],
    answer: 1,
    explanation: '红色研学重在把历史场景、革命精神与现实教育连接起来。'
  },
  {
    id: 'quiz-3',
    spot: '乌蒙山红军长征纪念点',
    question: '长征精神通常不包含以下哪一项？',
    options: ['坚定理想信念', '实事求是', '艰苦奋斗', '急功近利'],
    answer: 3,
    explanation: '长征精神强调理想信念、实事求是和艰苦奋斗，不包含急功近利。'
  }
]
...
const loadPassport = async () => {
  ...
  try {
    ...
    if (res) {
      passportData.value = {
        ...res,
        badges: Array.isArray(res.badges) ? res.badges.map(decorateProfileBadge) : []
      }
      if (res.quizCorrect != null) correctCount.value = res.quizCorrect
    }
  } catch (e) { /* 保留默认 */ }
}
...
.qr-badge { display: flex; align-items: center; justify-content: center; padding: 10px; border-radius: 10px; text-align: center; margin-bottom: 10px; }
.qr-ok { background: rgba(7,193,96,0.08); }
.qr-err { background: rgba(238,10,36,0.06); }
.qr-icon { width: 18px; height: 18px; display: block; margin-right: 6px; }
...
.pp-av-icon { width: 24px; height: 24px; display: block; }
.pp-info { flex: 1; }
...
.badge-icon-img { width: 22px; height: 22px; display: block; }
