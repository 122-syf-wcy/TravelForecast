  ...
  <view class="page">
    <view class="section">
      ...
      <view class="item">
        ...
        <switch class="item-sw" :checked="notifyOn" @change="onNotifyChange($event.detail.value)" color="#2A9D8F" />
      </view>
    </view>
    ...
    <view class="mask" v-if="showPrivacy" @tap="showPrivacy = false">
      <view class="popup popup-lg" @tap.stop>
        ...
        <scroll-view scroll-y class="privacy-scroll">
          ...
          <text class="popup-text">如有疑问，请前往“意见反馈”页面联系我们。</text>
        </scroll-view>
...
const notifyOn = ref(uni.getStorageSync('settingsNotify') !== false)
...
const onNotifyChange = (val) => {
  if (val) {
    if (!SUBSCRIBE_TMPL_IDS.length) {
      notifyOn.value = true
      uni.setStorageSync('settingsNotify', true)
      uni.showToast({ title: '消息通知已开启，待补充订阅模板后可接收微信提醒', icon: 'none' })
      return
    }
    // #ifdef MP-WEIXIN
    uni.requestSubscribeMessage({
      ...
      success: (res) => {
        ...
        notifyOn.value = accepted
        uni.setStorageSync('settingsNotify', accepted)
        if (!accepted) {
          uni.showToast({ title: '需要允许通知才能接收消息', icon: 'none' })
        }
      },
      fail: () => {
        notifyOn.value = false
        uni.setStorageSync('settingsNotify', false)
        uni.showToast({ title: '订阅消息授权失败', icon: 'none' })
      }
    })
    // #endif
    // #ifndef MP-WEIXIN
    notifyOn.value = val
    uni.setStorageSync('settingsNotify', val)
    // #endif
  } else {
    notifyOn.value = false
    uni.setStorageSync('settingsNotify', false)
...
const editProfile = () => {
  uni.setStorageSync('profileOpenEditor', true)
