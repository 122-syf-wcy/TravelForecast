    ...
    <view class="pf-hd" :style="{ paddingTop: stBar + 'px' }">
      <view class="pf-top">
        ...
        <view class="pf-set" @tap="goSet">
          <image class="set-gear-icon" :src="commonIcons.gear" mode="aspectFit" />
        </view>
      </view>
      ...
      <view class="user-card">
        <view class="uav" @tap="goLogin">
          <view class="uav-circle">
            ...
            <image v-else class="uav-fallback-icon" :src="commonIcons.ai" mode="aspectFit" />
          </view>
        </view>
        <view class="u-info">
    ...
    <view class="pp-sec" @tap="goPassport">
      <view class="pp-hd">
        <view class="pp-left">
          <view class="pp-icon-wrap">
            <image class="pp-icon-img" :src="commonIcons.passport" mode="aspectFit" />
          </view>
      ...
      <view class="pp-progress-wrap">
        <view class="pp-progress-bar">
          <view class="pp-progress-fill" :style="{ width: (badges.length ? (unlockedCount / badges.length * 100) : 0) + '%' }" />
        </view>
      </view>
      <view class="badge-grid">
        <view class="badge" v-for="(b, i) in badges" :key="i" :class="{ 'badge-off': !b.on }">
          <view class="badge-icon" :style="{ background: b.on ? b.bg : '' }">
            <image class="badge-icon-img" :src="b.icon" mode="aspectFit" />
          </view>
      ...
      <view class="order-grid">
        <view class="order-item" v-for="(o, i) in orderTypes" :key="i" @tap="goOrder(o)">
          <view class="oi-icon" :style="{ background: o.bg }">
            <image class="oi-icon-img" :src="o.icon" mode="aspectFit" />
      ...
      <view class="menu-item" v-for="(m, i) in menuList" :key="i" @tap="onMenu(m)">
        <view class="menu-l">
          <view class="menu-ico" :style="{ background: m.bg }">
            <image class="menu-ico-img" :src="m.icon" mode="aspectFit" />
          </view>
...
import { checkIn, getCheckInStatus, getFavorites, getCoupons, getFootprints } from '@/api/user'
import { fetchPassport } from '@/api/study'
import { fetchOrders } from '@/api/shop'
import { resolveAssetUrl } from '@/utils/url'
import {
  COMMON_ICONS,
  decorateProfileBadge,
  decorateProfileMenu,
  decorateProfileOrder
...
const AI_HISTORY_STORAGE_KEY = 'digitalHumanHistory'
const PROFILE_OPEN_EDITOR_KEY = 'profileOpenEditor'
...
const syncProfileState = async () => {
  ...
  await loadUserData(u.userId)
}

const refreshAiHistoryTip = () => {
  const history = uni.getStorageSync(AI_HISTORY_STORAGE_KEY)
  const count = Array.isArray(history) ? history.filter((item) => item && item.isMe && item.text).length : 0
  menuList.value = menuList.value.map((item) => (
    item.name === 'AI 对话历史'
      ? { ...item, tip: count > 0 ? `${count}条` : '' }
      : item
  ))
}
...
onShow(() => {
  syncProfileState()
  refreshAiHistoryTip()
  if (uni.getStorageSync(PROFILE_OPEN_EDITOR_KEY)) {
    uni.removeStorageSync(PROFILE_OPEN_EDITOR_KEY)
    if (isLogin.value) {
      setTimeout(() => { openProfileEditor() }, 120)
    } else {
      uni.showToast({ title: '请先登录后再编辑资料', icon: 'none' })
    }
  }
})
...
const loadUserData = async (userId) => {
  ...
  try {
    ...
    const [checkInRes, favRes, passportRes, ordersRes, couponRes, footprintRes] = await Promise.all([
      getCheckInStatus(userId).catch(() => null),
      getFavorites(userId).catch(() => []),
      fetchPassport(userId).catch(() => null),
      fetchOrders(userId, null).catch(() => []),
      getCoupons(userId).catch(() => []),
      getFootprints(userId).catch(() => [])
    ])
    ...
    if (Array.isArray(couponRes)) {
      dataRow.value[1].val = couponRes.filter(c => c.status === 'valid' || !c.status).length
    }
    ...
    // 更新足迹数
    if (Array.isArray(footprintRes)) {
      dataRow.value[3].val = footprintRes.length
    }
...
const saveProfile = async () => {
  ...
  const hasNewAvatar = !!tempAvatar.value && tempAvatar.value !== userAvatar.value && isTempAvatarFile(tempAvatar.value)
...
const onDataTap = (d) => {
  if (d.key === 'fav') {
    ...
    uni.navigateTo({ url: '/pages/profile/footprint' })
  } else if (d.key === 'coupon') {
    uni.navigateTo({ url: '/pages/profile/coupon' })
  }
}
...
const onMenu = (m) => {
  if (m.name === '我的行程') {
    ...
    uni.navigateTo({ url: '/pages/digital-human/index?history=1' })
  } else if (m.name === '地址管理') {
    ...
    uni.showToast({ title: '暂不支持该入口', icon: 'none' })
  }
}
...
.pf-set { padding: 4px; }
.set-gear-icon { width: 20px; height: 20px; display: block; }
...
.uav-fallback-icon { width: 26px; height: 26px; display: block; }
...
.pp-icon-img { width: 20px; height: 20px; display: block; }
...
.badge-icon-img { width: 18px; height: 18px; position: relative; z-index: 2; display: block; }
...
.oi-icon-img { width: 18px; height: 18px; display: block; position: relative; z-index: 2; }
...
.menu-ico-img { width: 15px; height: 15px; display: block; }
