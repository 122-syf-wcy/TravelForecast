  ...
  <view class="pp-page">
      ...
      <view class="pp-cover-body">
        <view class="pp-user">
          <view class="pp-user-avatar">
            <image class="pp-user-icon" :src="commonIcons.passport" mode="aspectFit" />
          </view>
    ...
    <view class="pp-progress-sec">
      <view class="pp-progress-hd">
        ...
        <text class="pp-progress-pct">{{ allBadges.length ? Math.round(unlockedCount / allBadges.length * 100) : 0 }}%</text>
      </view>
      <view class="pp-progress-bar">
        <view class="pp-progress-fill" :style="{ width: (allBadges.length ? (unlockedCount / allBadges.length * 100) : 0) + '%' }" />
      </view>
      ...
      <view class="badge-list">
        <view class="badge-card" v-for="(b, i) in allBadges" :key="i" :class="{ 'badge-locked': !b.unlocked }">
          <view class="bc-icon" :style="{ background: b.unlocked ? b.bg : '' }">
            <image class="bc-icon-img" :src="b.icon" mode="aspectFit" />
          </view>
          <view class="bc-info">
          ...
          <view class="bc-status">
            <view class="bc-status-chip">
              <image class="bc-status-icon" :src="b.unlocked ? commonIcons.checkCircle : commonIcons.lock" mode="aspectFit" />
    ...
    <view class="pp-stamp-sec">
      ...
      <view class="stamp-timeline" v-if="stamps.length > 0">
        <view class="stamp-item" v-for="(s, i) in stamps" :key="i">
          ...
          <view class="stamp-card">
            ...
            <view class="sc-stamp">
              <image class="sc-stamp-icon" :src="commonIcons.checkCircle" mode="aspectFit" />
      ...
      <view class="stamp-empty" v-else>
        <image class="stamp-empty-icon" :src="commonIcons.location" mode="aspectFit" />
    ...
    <view class="pp-quiz-sec" @tap="goQuiz">
      <view class="quiz-entry">
        <view class="qe-left">
          <view class="qe-icon">
            <image class="qe-icon-img" :src="badgeVisuals['答题学霸'].icon" mode="aspectFit" />
          </view>
          <view class="qe-info">
...
import { COMMON_ICONS, PROFILE_BADGE_VISUALS, decorateProfileBadge } from '@/utils/icon-catalog'

const userName = ref('旅行者')
const userId = ref('000001')
const quizCorrect = ref(0)
const commonIcons = COMMON_ICONS
const badgeVisuals = PROFILE_BADGE_VISUALS

const passportData = ref({ badges: [], totalPoints: 0, checkedSpots: 0 })

const defaultBadgeDefs = [
  { name: '初来乍到', desc: '首次登录小程序' },
  { name: '三线学者', desc: '参观三线建设博物馆' },
  { name: '美食猎人', desc: '打卡 3 个美食景点' },
  { name: '凉都达人', desc: '打卡 5 个景区' },
  { name: '文创大师', desc: '购买 1 件文创商品' },
  { name: '答题学霸', desc: '答对 10 道研学题' },
  { name: '全景探索', desc: '打卡全部景区' },
  { name: '凉都研学大师', desc: '解锁全部徽章' }
]
const allBadges = ref(defaultBadgeDefs.map(b => decorateProfileBadge({ ...b, unlocked: false })))
...
onMounted(async () => {
  ...
  if (u && u.userId) {
    ...
    userId.value = String(u.userId).padStart(6, '0')
  } else {
    userName.value = '未登录'
    userId.value = '------'
  }
  try {
    ...
    if (uid) {
      ...
      if (res) {
        ...
        if (Array.isArray(res.badges) && res.badges.length > 0) {
          const serverMap = {}
          res.badges.forEach(b => { if (b.name) serverMap[b.name] = b })
          allBadges.value = defaultBadgeDefs.map(def => {
            const server = serverMap[def.name]
            return decorateProfileBadge({
              ...def,
              ...(server || {}),
              unlocked: server ? !!server.unlocked : false
            })
          })
          res.badges.forEach(b => {
            if (b.name && !defaultBadgeDefs.find(d => d.name === b.name)) {
              allBadges.value.push(decorateProfileBadge({ ...b, unlocked: !!b.unlocked }))
            }
          })
        }
...
.pp-user-icon { width: 28px; height: 28px; display: block; }
.pp-user-info { flex: 1; }
...
.bc-icon-img { width: 20px; height: 20px; display: block; }
...
.bc-status-chip { display: flex; align-items: center; }
.bc-status-icon { width: 12px; height: 12px; display: block; margin-right: 3px; }
...
.sc-stamp { display: inline-flex; align-items: center; background: rgba(42,157,143,0.08); padding: 2px 8px; border-radius: 10px; }
.sc-stamp-icon { width: 12px; height: 12px; display: block; margin-right: 4px; }
.sc-stamp-t { font-size: 10px; color: #2A9D8F; font-weight: 600; }

.stamp-empty { display: flex; flex-direction: column; align-items: center; padding: 24px 0; }
.stamp-empty-icon { width: 40px; height: 40px; display: block; margin-bottom: 10px; }
...
.qe-icon-img { width: 18px; height: 18px; display: block; }
