  ...
  <view class="guide-page">
    ...
    <view class="map-ctrls">
      <view class="m-btn" @tap="moveToMe">
        <image class="m-btn-icon" src="/static/icons/location-dark.svg" mode="aspectFit" />
        <text class="m-btn-t">定位</text>
      </view>
      <view class="m-btn m-btn-ai" @tap="startGuide">
        <image class="m-btn-icon" :src="commonIcons.ai" mode="aspectFit" />
        <text class="m-btn-t m-btn-t-light">讲解</text>
      </view>
      <view class="m-btn m-btn-quiz" v-if="studyMode" @tap="openQuiz">
        <image class="m-btn-icon" :src="commonIcons.question" mode="aspectFit" />
        <text class="m-btn-t m-btn-t-light">答题</text>
      </view>
    </view>
    ...
    <view class="arrive" v-if="showArrive" @tap="onArriveTap">
      <view class="arrive-inner">
        <view class="arrive-av">
          <image class="arrive-av-icon" :src="commonIcons.ai" mode="aspectFit" />
        </view>
...
import { ensureMiniProgramPermission, hasMiniProgramPermission } from '@/utils/permissions'
...
const quiz = computed(() => {
  if (quizBank.value.length > 0 && quizIdx.value < quizBank.value.length) {
    ...
    return {
      ...
      answer: q.answer != null ? Number(q.answer) || 0 : 0
    }
  }
  return {
    id: null,
...
onMounted(() => {
  ...
  hasMiniProgramPermission('scope.userLocation').then((granted) => {
    showLocation.value = granted
  })
  loadSpots()
})

let locationTimer = null
const ARRIVE_DISTANCE_M = 500
const arrivedPois = ref(new Set())
...
const checkProximity = () => {
  uni.getLocation({
    type: 'gcj02',
    success: (loc) => {
      for (const poi of poiList.value) {
        if (arrivedPois.value.has(poi.id)) continue
        const dist = haversineDistance(loc.latitude, loc.longitude, poi.latitude, poi.longitude)
        if (dist <= ARRIVE_DISTANCE_M) {
          arrivedPois.value.add(poi.id)
          arriveMsg.value = `你已到达${poi.title}附近，是否开启 AI 讲解？`
          curPoi.value = poi
          showArrive.value = true
          setTimeout(() => { showArrive.value = false }, 8000)
          break
        }
      }
    }
...
const startLocationWatch = () => {
  ...
  checkProximity()
  locationTimer = setInterval(checkProximity, 15000)
}
...
const toggleStudy = async () => {
  ...
  if (studyMode.value) {
    try {
      await ensureMiniProgramPermission('scope.userLocation', '研学模式需要定位权限，以便在你到达景区时自动提醒。')
      startLocationWatch()
    } catch (e) {
      uni.showToast({ title: '未授予定位权限，到达提醒不可用', icon: 'none' })
    }
  } else {
    stopLocationWatch()
...
const moveToMe = async () => {
  try {
    await ensureMiniProgramPermission('scope.userLocation', '开启定位权限后，才能在地图里展示你的位置并快速定位。')
  } catch (err) {
    uni.showToast({ title: err.message || '未授予定位权限', icon: 'none' })
    return
  }
...
const startGuide = () => {
  ...
  if (poi) {
    ...
    const params = [`scenicName=${encodeURIComponent(poi.title)}`, `initMsg=${query}`]
    if (poi.id) params.unshift(`scenicId=${encodeURIComponent(poi.id)}`)
    uni.navigateTo({ url: `/pages/digital-human/index?${params.join('&')}` })
  } else {
...
const onMarkerTap = (e) => {
  ...
  const poi = poiList.value.find(p => String(p.id) === String(mid))
...
.map-ctrls { position: absolute; right: 12px; bottom: 40px; display: flex; flex-direction: column; }
.m-btn {
...
.m-btn-icon { width: 18px; height: 18px; display: block; margin-bottom: 4px; }
.m-btn-t { font-size: 10px; color: #666; }
.m-btn-t-light { color:#fff; }
...
.arrive-av-icon { width: 20px; height: 20px; display: block; }
.arrive-body { flex: 1; }
