<template>
  ...
  <view class="pay-mask" v-if="visible" @tap.stop>
    ...
    <view class="pay-panel" v-if="stage === 'confirm'" @tap.stop>
      ...
      <view class="pay-methods">
        <view class="pay-method" :class="{ 'pm-active': payMethod === 'wechat' }" @tap="payMethod = 'wechat'">
          ...
          <view class="pm-check" :class="{ 'pm-checked': payMethod === 'wechat' }">
            <image class="pm-check-icon" v-if="payMethod === 'wechat'" :src="commonIcons.checkCircle" mode="aspectFit" />
          </view>
        </view>
        <view class="pay-method" :class="{ 'pm-active': payMethod === 'points' }" @tap="payMethod = 'points'" v-if="showPointsPay">
          ...
          <view class="pm-check" :class="{ 'pm-checked': payMethod === 'points' }">
            <image class="pm-check-icon" v-if="payMethod === 'points'" :src="commonIcons.checkCircle" mode="aspectFit" />
          </view>
        </view>
      </view>
      <!-- 确认按钮 -->
      ...
      <text class="pay-safe-tip">{{ safeTipText }}</text>
    </view>
    ...
    <view class="pay-result-overlay" v-if="stage === 'result'" @tap.stop>
      <view class="pay-result-box">
        <view class="result-icon" :class="paySuccess ? 'result-ok' : 'result-fail'">
          <image class="result-icon-img" :src="paySuccess ? commonIcons.checkCircle : commonIcons.xCircle" mode="aspectFit" />
        </view>
...
import { payWithPoints } from '@/api/user'
...
const onConfirm = async () => {
  // 黔豆支付：仍走密码演示（doPayment 里调 payWithPoints）
  if (payMethod.value !== 'wechat') {
    if (props.pointsBalance < pointsNeeded.value) {
      uni.showToast({ title: `黔豆不足，需 ${pointsNeeded.value} 黔豆`, icon: 'none' })
      return
    }
  }
  password.value = ''
...
const doPayment = async () => {
  ...
  try {
    ...
    if (payMethod.value === 'points') {
      if (userId && props.orderId) {
        try {
          ...
          if (!res || res.success !== true) throw new Error(res?.message || '黔豆支付失败')
          const usedPoints = Number(res.usedPoints ?? pointsNeeded.value)
          const remainingPoints = Number(res.remainingPoints ?? Math.max(props.pointsBalance - usedPoints, 0))
          paySuccess.value = true
          resultMsg.value = `已扣除 ${usedPoints} 黔豆，剩余 ${remainingPoints} 黔豆`
          ...
          try {
            const { updateOrderStatus } = await import('@/api/shop')
            await updateOrderStatus(props.orderId, 'paid')
          } catch (_) {}
        }
      }

      const usedPoints = pointsNeeded.value
      const remainingPoints = Math.max(props.pointsBalance - usedPoints, 0)
      paySuccess.value = true
      resultMsg.value = `模拟黔豆支付成功，已抵扣 ${usedPoints} 黔豆`
      successPayload.value = {
        orderId: props.orderId,
        orderNo: props.orderNo,
        method: 'points',
    ...
    paySuccess.value = true
    resultMsg.value = props.orderId ? '模拟支付成功，订单已更新为待使用' : '模拟支付成功'
    successPayload.value = {
      orderId: props.orderId,
      orderNo: props.orderNo,
      method: 'wechat',
...
const onResultDone = () => {
  if (paySuccess.value) {
    emit('success', successPayload.value || {
      orderId: props.orderId,
      orderNo: props.orderNo,
      method: payMethod.value,
      simulated: payMethod.value === 'wechat'
    })
  } else {
    // 重试 → 回到确认页
    password.value = ''
...
const onViewOrder = () => {
  ...
  const targetUrl = '/pages/order/list?tab=2'
  if (currentPage && currentPage.route === 'pages/order/list') {
...
.pm-check-icon { width: 14px; height: 14px; display: block; }
...
.result-icon-img { width: 44px; height: 44px; display: block; }
