...
# 可观测性：Prometheus 指标 + OpenTelemetry trace
# 任一依赖未安装都不会阻塞启动，向后兼容旧的运行环境。
try:
...
# 服务启动时间，供 /health 与 / 返回运行时长
SERVICE_STARTED_AT = datetime.now()

# Prometheus /metrics 端点（默认开启，可通过 PROMETHEUS_METRICS=0 关闭）
if Instrumentator is not None and os.environ.get("PROMETHEUS_METRICS", "1") == "1":
    try:
        ...
        logger.warning("Prometheus 指标挂载失败: {}", exc)

# OpenTelemetry 链路追踪（默认开启，需 OTEL_EXPORTER_OTLP_ENDPOINT 指向 collector）
_otlp_endpoint = os.environ.get("OTEL_EXPORTER_OTLP_ENDPOINT")
if _OTEL_AVAILABLE and _otlp_endpoint:
    try:
        resource = Resource.create({SERVICE_NAME: os.environ.get("OTEL_SERVICE_NAME", "travel-prediction-service")})
        ...
        logger.info("OpenTelemetry tracing 已启用，OTLP={}", _otlp_endpoint)
    except Exception as exc:  # noqa: BLE001
...
@app.get("/")
async def root():
    """服务根路径，返回基础元信息，供网关/运维快速识别。"""
    return {
        "service": "travel-prediction-service",
        "version": app.version,
        "status": "running",
        "startedAt": SERVICE_STARTED_AT.isoformat(timespec="seconds"),
        "scenicCount": len(SCENIC_NAMES),
        "endpoints": [
            "/health",
            "/api/prediction/flow/{scenic_id}",
            "/api/prediction/total",
            "/api/prediction/hourly/{scenic_id}",
            "/api/prediction/hourly/total",
            "/api/prediction/train/{scenic_id}",
            "/api/prediction/models/info",
        ],
    }
...
async def health_check():
    """
    健康检查端点，用于网关、Java 客户端与监控探活。
    返回模型状态而不依赖外部 DB，避免探活被副作用阻塞。
    """
    uptime_seconds = (datetime.now() - SERVICE_STARTED_AT).total_seconds()
    models_status = {
        "arima": getattr(arima_model, "is_ready", True),
        "lstm": getattr(lstm_model, "is_ready", True),
        "dual_stream": getattr(dual_stream_model, "is_ready", True),
    }
    ...
    return {
        ...
        "scenicSupported": list(SCENIC_NAMES.keys()),
    }
...
async def get_total_prediction(
    ...
    try:
        ...
        # 聚合预测的置信度以所选模型的实际评估置信度为基准；
        # 避免在代码中对全域总量写死 0.95，使指标始终可追溯到 model_metrics.json。
        if model == "arima":
            aggregate_confidence = arima_model.get_confidence()
        elif model == "lstm":
            aggregate_confidence = lstm_model.get_confidence()
        else:
            aggregate_confidence = dual_stream_model.get_confidence()
