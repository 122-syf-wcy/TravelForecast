...
from models.metrics_registry import get_metrics
...
class DualStreamHybridModel:
    ...
    def get_confidence(self) -> float:
        """
        置信度统一从 `experiments/results/model_metrics.json` 读取。
        如果尚未执行真实评估，将返回保守默认值（见 metrics_registry._DEFAULTS），
        并在加载阶段打印一次 warn 日志，避免向业务方暴露"写死的 0.92"。
        """
        return get_metrics(self.MODEL_NAME).confidence

    def get_accuracy(self) -> float:
        return get_metrics(self.MODEL_NAME).accuracy
