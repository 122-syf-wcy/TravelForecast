...
public class ShopController {
    ...
    public Result<String> addToCart(@RequestBody Map<String, Object> body) {
        ...
        String spec = body.containsKey("spec") && body.get("spec") != null ? body.get("spec").toString() : null;

        LambdaQueryWrapper<Cart> qw = new LambdaQueryWrapper<>();
        qw.eq(Cart::getUserId, userId).eq(Cart::getProductId, productId);
        if (spec != null && !spec.isBlank()) {
            qw.eq(Cart::getSpec, spec);
        } else {
            qw.isNull(Cart::getSpec);
        }
    ...
    public Result<MpOrder> createOrder(@RequestBody Map<String, Object> body) {
        ...
        for (Map<String, Object> item : items) {
            ...
            oi.setCartId(item.containsKey("cartId") && item.get("cartId") != null ? Long.valueOf(item.get("cartId").toString()) : null);
            ...
            oi.setSpec(item.containsKey("spec") && item.get("spec") != null ? item.get("spec").toString() : null);
    ...
    public Result<String> updateOrderStatus(@PathVariable Long id, @RequestBody Map<String, String> body) {
        ...
        if ("paid".equals(newStatus)) {
            LambdaQueryWrapper<MpOrderItem> itemQw = new LambdaQueryWrapper<>();
            itemQw.eq(MpOrderItem::getOrderId, id);
            List<MpOrderItem> items = orderItemMapper.selectList(itemQw);
            List<Long> cartIds = items.stream()
                    .map(MpOrderItem::getCartId)
