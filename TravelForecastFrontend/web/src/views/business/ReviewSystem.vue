...
const fetchKeywords = async () => {
  try {
    devLog('开始提取关键词...')
    devLog('当前评论数据数量:', reviewData.value.length)
    ...
    reviewData.value.forEach((review, index) => {
      ...
      devLog(`评论${index}: 评分=${review.rating}, 类型=${type}, 标签=`, tags)
    ...
    devLog('提取到的所有关键词:', keywords)
    devLog('当前筛选类型:', wordCloudType.value)
...
const loadReviews = async () => {
  try {
    ...
    devLog('商家端评论数据已加载:', reviewData.value)
