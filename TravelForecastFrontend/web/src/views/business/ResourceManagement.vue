...
const loadScenicData = async () => {
  try {
    ...
    devLog('商家资料响应:', profileResponse.data)
    ...
    if (response.data.code === 1 || response.data.code === 200) {
      ...
      devLog('转换后的景区数据:', scenicResources.value)
...
const loadActivities = async () => {
  try {
    ...
    devLog('加载活动数据成功:', activityResources.value)
  } catch (error: any) {
...
const loadFacilities = async () => {
  try {
    ...
    devLog('加载设施数据成功:', facilityResources.value)
  } catch (error: any) {
...
const loadSubSpots = async () => {
  try {
    ...
    devLog('加载子景点成功:', subSpotResources.value)
  } catch (error: any) {
