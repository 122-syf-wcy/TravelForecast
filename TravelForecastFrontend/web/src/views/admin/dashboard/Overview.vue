...
const initCharts = () => {
  ...
  setTimeout(() => {
    try {
      devLog('开始初始化图表', trendChartRef.value, distributionChartRef.value)
...
const initTrendChart = () => {
  ...
  devLog(`趋势图表容器尺寸: ${containerWidth}x${containerHeight}`)
...
const initDistributionChart = async () => {
  ...
  devLog(`分布图表容器尺寸: ${containerWidth}x${containerHeight}`)
...
const updateTrendChart = (data: any) => {
  if (!trendChart || !data) {
    devLog('无法更新趋势图表', { trendChart: !!trendChart, data: !!data })
    return
  }
  
  try {
    devLog('更新趋势图表数据:', data)
    const option = {
      xAxis: [{
        data: data.dates || []
      }],
