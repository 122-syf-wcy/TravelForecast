            ...
            <div v-for="(feat, index) in config.features" :key="index" class="feat-edit-card">
              <el-form label-width="80px">
                <el-form-item label="图标">
                  <el-input v-model="feat.icon" placeholder="SVG 图标名，如 chart" maxlength="24" />
                </el-form-item>
        ...
        <el-tab-pane label="典型场景" name="scenarios">
          <div class="space-y-4">
            <div v-for="(sc, index) in config.scenarios" :key="index" class="sc-edit-card">
              <el-form label-width="80px">
                <el-form-item label="图标">
                  <el-input v-model="sc.icon" placeholder="SVG 图标名，如 traffic" maxlength="24" />
                </el-form-item>
                <el-form-item label="标题">
...
const config = reactive({
  ...
  features: [
    { id: 1, icon: 'sparkle', title: '智能客流预测', description: '基于LSTM-ARIMA混合模型，精准预测景区未来客流趋势' },
    { id: 2, icon: 'map', title: '行程规划', description: 'AI算法优化游览路线，避开高峰节省时间' },
    { id: 3, icon: 'chart', title: '实时数据分析', description: '3D可视化大屏，实时监控景区运营状态' }
  ],
  highlights: [
    {
      id: 1,
  ...
  scenarios: [
    { id: 1, icon: 'traffic', title: '高峰错峰与导流', desc: '节假日提前预警并优化入园时段，减少拥堵' },
    { id: 2, icon: 'bus', title: '交通接驳编排', desc: '预测客流峰谷，动态调整公交与摆渡车频次' },
    { id: 3, icon: 'shield', title: '承载与安全保障', desc: '超阈值实时告警，联动安保与广播提示' }
  ],
  testimonials: [
