...
const submitEdit = async () => { await editRef.value?.validate?.();
  devLog('准备保存, editForm:', JSON.parse(JSON.stringify(editForm.value)))
...
const uploadBanner = async (option: any) => {
  ...
  try {
    ...
    devLog('设置image URL(保存公共URL):', publicUrl)
    editForm.value.image = publicUrl
    editPreview.value = previewUrl
    devLog('当前editForm:', JSON.parse(JSON.stringify(editForm.value)))
...
const loadList = async () => {
  ...
  const data = res?.data || res
  devLog('后端返回的分页数据:', data)
  bannerList.value = data?.records || data?.list || []
  devLog('渲染的列表数据:', bannerList.value)
}
