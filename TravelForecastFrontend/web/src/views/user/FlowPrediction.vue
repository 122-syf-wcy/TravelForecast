...
const generateBasicSuggestions = () => {
  ...
  bestDays.forEach((day: any) => {
    suggestions += `• ${day.date}（${day.weekday}）- 预计客流${day.expectedFlow}人，${day.weatherCondition === 'sunny' ? '晴天晴天' : day.weatherCondition === 'rainy' ? '雨天雨天' : '多云多云'}，拥挤度${day.congestionLevel === 'low' ? '较低' : day.congestionLevel === 'medium' ? '适中' : '较高'}\n`
  })
