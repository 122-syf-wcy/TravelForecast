            ...
            <div class="scenic-info-section">
              <div class="scenic-main-image">
                ...
                <div class="image-badges">
                  <span class="badge level-badge"><SvgIcon name="mountain" /> 4A景区</span>
                  <span class="badge hot-badge" v-if="currentScenic.recommended"><SvgIcon name="flame" /> 推荐</span>
                </div>
              </div>
              ...
              <div class="suggestion-content">
                <div class="suggestion-header">
                  <span class="suggestion-title">
                    <SvgIcon :name="currentScenic.recommended ? 'sparkle' : 'warning'" class="inline-block mr-1" />{{ currentScenic.recommended ? '适合游览' : '温馨提示' }}
                  </span>
                  <span 
                  ...
                  >
                    <span class="trend-icon"><SvgIcon :name="currentScenic.trend === 'rising' ? 'trend-up' : currentScenic.trend === 'falling' ? 'trend-down' : 'arrow-right'" /></span>
      ...
      <div class="emergency-dialog-content">
        ...
        <div class="rescue-type-section">
          ...
          <div class="rescue-type-grid">
            ...
            >
              <span class="rescue-icon"><SvgIcon name="medical" /></span>
            ...
            >
              <span class="rescue-icon"><SvgIcon name="shield" /></span>
            ...
            >
              <span class="rescue-icon"><SvgIcon name="map" /></span>
            ...
            >
              <span class="rescue-icon"><SvgIcon name="phone" /></span>
    ...
    <el-dialog v-model="ticketDialogVisible" title="" width="520px" class="ticket-dialog" :show-close="false">
      ...
      <template #header>
        <div class="ticket-dialog-header">
          ...
          <div class="header-text">
            <h3 class="header-title"><SvgIcon name="ticket" /> 在线购票</h3>
      ...
      <div class="ticket-dialog-content">
        ...
        <div v-if="currentScenic" class="scenic-info-card">
          ...
          <div class="scenic-info">
            ...
            <div class="scenic-tags">
              <span class="tag"><SvgIcon name="mountain" /> 4A景区</span>
              <span class="tag"><SvgIcon name="star" /> 热门推荐</span>
            </div>
          </div>
        </div>
        ...
        <div class="form-section">
          ...
          <div class="ticket-type-grid">
            ...
            >
              <div class="ticket-type-icon"><SvgIcon name="user" /></div>
            ...
            >
              <div class="ticket-type-icon"><SvgIcon name="child" /></div>
              <div class="ticket-type-info">
...
const getWeatherEmoji = (condition: string): string => {
  ...
  if (conditionLower.includes('晴')) return 'sun'
  if (conditionLower.includes('多云')) return 'cloud-sun'
  if (conditionLower.includes('阴')) return 'cloud'
  if (conditionLower.includes('雨')) return 'rain'
  if (conditionLower.includes('雪')) return 'snow'
  if (conditionLower.includes('雾')) return 'fog'
  if (conditionLower.includes('风')) return 'wind'
  return 'sun'
}
...
const loadActivities = async () => {
  devLog('[活动] 开始加载活动, selectedScenic:', selectedScenic.value)
  ...
  try {
    const scenicId = scenicNameToId[selectedScenic.value]
    devLog('[活动] 景区ID映射:', selectedScenic.value, '->', scenicId)
    ...
    devLog('[活动] 开始调用API: getOngoingActivities, scenicId=', scenicId, 'page=', activityPage.value, 'size=', activitySize.value)
    const res: any = await getOngoingActivities({
      scenicId,
      page: activityPage.value,
      size: activitySize.value
    })
    devLog('[活动] API返回原始数据:', res)
    
    const data = res?.data || res
    devLog('[活动] 解析后的数据:', data)
    
    activities.value = data.activities || []
    devLog('[活动] 解析后的活动数据:', activities.value)
    devLog('[活动] 活动数量:', activities.value.length)
  } catch (error) {
...
const loadAnnouncements = async () => {
  devLog('[公告] 开始加载公告, selectedScenic:', selectedScenic.value)
  ...
  try {
    const scenicId = scenicNameToId[selectedScenic.value]
    devLog('[公告] 景区ID映射:', selectedScenic.value, '->', scenicId)
    ...
    devLog('[公告] 开始调用API: getScenicAnnouncements, scenicId=', scenicId)
    const res: any = await getScenicAnnouncements({ scenicId })
    devLog('[公告] API返回数据:', res)
    
    const data = res?.data || res
    announcements.value = data.announcements || []
    
    devLog('[公告] 解析后的公告数据:', announcements.value)
    devLog('[公告] 公告数量:', announcements.value.length)
  } catch (error) {
...
const loadSystemNotifications = async () => {
  try {
    ...
    if (res.code === 200) {
      ...
      devLog('[系统通知] 加载成功:', systemNotifications.value.length, '条')
    }
  } catch (error) {
...
const handleSystemNotificationClick = async (notification: any) => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      devLog('[系统通知] 标记已读成功:', notification.id)
