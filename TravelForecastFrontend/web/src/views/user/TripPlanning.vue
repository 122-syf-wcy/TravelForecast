            ...
            <div class="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
              ...
              <div class="flex gap-2">
                <el-button size="small" type="success" plain round @click="triggerPlanNarration">
                  <SvgIcon name="graduation" class="mr-1" />数字人讲解
                </el-button>
                ...
                <div class="activity-content">
                  <div class="activity-header">
                    <div class="activity-icon" :class="'icon-' + activity.type">
                      <SvgIcon :name="getActivityIcon(activity.type)" />
                    </div>
                  ...
                  <div v-if="activity.type === 'scenic' && activity.details" class="activity-extra scenic-extra">
                    <span v-if="activity.details.ticket" class="extra-tag">
                      <SvgIcon name="ticket" /> {{ activity.details.ticket }}
                    </span>
                    ...
                    <span v-if="activity.details.crowdLevel" class="extra-tag" :class="'crowd-' + activity.details.crowdLevel">
                      <SvgIcon name="users" /> {{ activity.details.crowdLevel }}
                    </span>
                  </div>
                  ...
                  <div v-if="activity.type === 'food' && activity.details" class="activity-extra food-extra">
                    <span v-if="activity.details.price" class="extra-tag">
                      <SvgIcon name="money" /> 人均{{ activity.details.price }}
                    </span>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <ul class="tips-list grid grid-cols-1 md:grid-cols-2 gap-4">
              <li v-for="(tip, index) in (tripPlan.tips || defaultTips)" :key="index" class="tip-item flex items-start gap-3 p-3 bg-orange-50 rounded-lg text-sm text-gray-700 leading-relaxed border border-orange-100">
                <span class="tip-icon mt-0.5 text-orange-500"><SvgIcon name="lightbulb" /></span>
...
const budgetItems = computed(() => {
  ...
  return [
    { name: '景点门票', value: tripPlan.value.ticketCost, icon: 'ticket', color: '#36DBFF', percentage: Math.round((tripPlan.value.ticketCost / total) * 100) },
    { name: '交通费用', value: tripPlan.value.transportationCost, icon: 'car', color: '#9F87FF', percentage: Math.round((tripPlan.value.transportationCost / total) * 100) },
    { name: '餐饮费用', value: tripPlan.value.foodCost, icon: 'food', color: '#FF7ECB', percentage: Math.round((tripPlan.value.foodCost / total) * 100) },
    { name: '住宿费用', value: tripPlan.value.accommodationCost, icon: 'hotel', color: '#FFC861', percentage: Math.round((tripPlan.value.accommodationCost / total) * 100) },
    { name: '其他费用', value: tripPlan.value.otherCost, icon: 'package', color: '#4FFBDF', percentage: Math.round((tripPlan.value.otherCost / total) * 100) }
  ]
})
...
const downloadPlan = () => {
  ...
  const reportHTML = '<!DOCTYPE html>' +
    ...
    '<h1 class="report-title">六盘水精彩之旅</h1>' +
