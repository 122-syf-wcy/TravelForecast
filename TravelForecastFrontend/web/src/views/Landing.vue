      ...
      <div class="section-container">
        ...
        <div class="experience-grid">
          <div v-for="exp in experiences" :key="exp.id" class="experience-card reveal" @click="goToExperience(exp.path)">
            <div class="exp-icon"><SvgIcon :name="exp.icon" /></div>
      ...
      <div class="section-container">
        ...
        <div class="features-grid">
          <div v-for="feature in features" :key="feature.id" class="feature-card reveal">
            <div class="feature-icon"><SvgIcon :name="feature.icon" /></div>
      ...
      <div class="section-container">
        ...
        <div class="scenarios-grid">
          <div v-for="s in scenarios" :key="s.id" class="scenario-card reveal">
            <div class="scenario-icon"><SvgIcon :name="s.icon" /></div>
...
const features = ref([
  { id: 1, icon: 'chart', title: '智能预测', description: '基于双流融合模型的客流量预测准确率高达92%' },
  { id: 2, icon: 'map', title: '实时监控', description: '景区人流实时监控热力图可视化展示' },
  { id: 3, icon: 'trend-up', title: '数据分析', description: '多维度数据分析洞察旅游趋势变化' },
  { id: 4, icon: 'bell', title: '预警推送', description: '智能预警系统及时推送拥挤预警信息' },
  { id: 5, icon: 'phone', title: '多端适配', description: '支持PC平板手机多终端访问' },
  { id: 6, icon: 'bot', title: 'AI助手', description: '智能问答助手随时解答旅游相关问题' }
])

const experiences = ref([
  { id: 1, icon: 'chart', title: '客流预测', desc: '体验AI驱动的景区客流量预测功能', path: '/user/prediction' },
  { id: 2, icon: 'map', title: '实时服务', desc: '查看景区实时人流热力分布', path: '/user/service' },
  { id: 3, icon: 'bot', title: 'AI问答', desc: '与智能助手对话获取旅游建议', path: '/user/ai' }
])
...
const scenarios = ref([
  { id: 1, icon: 'building', title: '景区管理', desc: '实时掌握客流动态优化资源配置提升游客体验' },
  { id: 2, icon: 'car', title: '交通调度', desc: '预测高峰时段提前部署交通疏导方案' },
  { id: 3, icon: 'hotel', title: '酒店运营', desc: '预测入住需求动态调整房价策略' },
  { id: 4, icon: 'megaphone', title: '营销推广', desc: '分析游客画像精准投放营销内容' }
])
...
const fetchLandingData = async () => {
  try {
    ...
    devLog('Landing API response:', res)
    
    let data = res.data
    if (res.data?.code === 200 && res.data?.data) {
      data = res.data.data
    }
    devLog('Parsed landing data:', data)
    
    // 更新 stats 数据
    if (data && data.stats && data.stats.length > 0) {
      devLog('Stats from API:', JSON.stringify(data.stats))
    ...
    devLog('Landing API failed', e)
  }
  
  // 总是获取轮播图数据
  try {
    const bannersRes = await contentApi.getBanners()
    devLog('Banners API response:', bannersRes.data)
    ...
    if (bannersData && bannersData.length > 0) {
      ...
      devLog('Banners loaded:', banners.value.length, 'Hero image:', heroImage.value)
      // 启动轮播
      if (banners.value.length > 1) {
        startCarousel()
      }
    ...
    devLog('Banners API failed', e)
  }
  
  // 获取实景预览数据
  try {
    const showcaseRes = await contentApi.getShowcases()
    devLog('Showcases API response:', showcaseRes)
    ...
    devLog('Parsed showcase data:', showcaseData)
    ...
    if (showcaseData && Array.isArray(showcaseData) && showcaseData.length > 0) {
      ...
      devLog('Showcases loaded:', showcases.value.length, showcases.value)
    } else {
      showcases.value = []
      devLog('No showcase data found')
    }
  } catch (e) {
    devLog('Showcases API failed', e)
    showcases.value = []
  }
}
