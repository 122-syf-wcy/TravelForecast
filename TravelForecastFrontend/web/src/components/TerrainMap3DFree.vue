...
const loadScenicData = async () => {
  try {
    ...
    if (list && list.length > 0) {
      ...
      devLog(`加载 ${scenicSpots.value.length} 个景区数据`)
    } else {
...
onMounted(async () => {
  ...
  devLog('容器初始尺寸:', { width: rect.width, height: rect.height })
  
  if (rect.width === 0 || rect.height === 0) {
    console.warn('容器初始尺寸为0，延迟500ms后再初始化...')
...
const initMap = () => {
  ...
  try {
    ...
    devLog('高德地图创建成功（卫星3D地形）')
    devLog('地图中心:', map.getCenter())
    devLog('缩放级别:', map.getZoom())
    ...
    map.on('complete', () => {
      devLog('地图加载完成！')
      devLog(`开始添加 ${scenicSpots.value.length} 个景区标记`)
      ...
      scenicSpots.value.forEach((spot, index) => {
        if (!spot.coordinates || isNaN(spot.coordinates[0]) || isNaN(spot.coordinates[1]) ||
          ...
          console.warn(`  跳过无效坐标的景区: ${spot.name}`)
          return
        }
        
        devLog(`  添加标记 ${index + 1}/${scenicSpots.value.length}: ${spot.name} [${spot.coordinates[0]}, ${spot.coordinates[1]}]`)
        ...
        devLog(`标记 ${spot.name} 已添加到地图`)
      })
      
      devLog(`所有 ${scenicSpots.value.length} 个景区标记添加完成！`)
    })
