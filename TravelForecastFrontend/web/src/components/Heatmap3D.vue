...
const loadHeatmapData = async (scenicId: number) => {
  ...
  loading.value = true
  devLog(`正在加载景区ID ${scenicId} 的热力图数据...`)
  ...
  try {
    ...
    if (data && data.time && data.areas && data.data) {
      ...
      devLog(`景区ID ${scenicId} 热力图数据加载成功，数据点数: ${heatmapData.value.data.length}`)
      if (data.avgVisitorCount) {
        devLog(`   平均访客数: ${data.avgVisitorCount}`)
      }
    } else {
    ...
    console.warn(`加载景区ID ${scenicId} 热力图数据失败，使用默认数据:`, error)
    // 不显示错误提示，静默使用默认数据
    heatmapData.value = defaultData
  } finally {
    loading.value = false
...
const resize = () => {
  ...
  if (chart && chartRef.value) {
    devLog('当前容器尺寸:', chartRef.value.clientWidth, 'x', chartRef.value.clientHeight)
    chart.resize()
    // 延迟再次resize确保生效
    setTimeout(() => {
      chart?.resize()
...
watch(() => props.scenicId, async (newId, oldId) => {
  devLog(`景区切换: ${oldId} -> ${newId}`)
  if (newId) {
    devLog(`正在加载景区ID ${newId} 的客流热力图数据`)
    // 根据景区ID加载不同的热力图数据
