...
const unlockAudioContext = () => {
  ...
  if (ctx.state === 'suspended') {
    ctx.resume().then(() => devLog('[Audio] 嵌入式音频上下文已解锁'))
  }
}
...
watch(() => digitalHumanStore.pendingNarration, (prompt) => {
  ...
  devLog('[嵌入式导览] 触发自动讲解:', narrationPrompt.substring(0, 40) + '...')
...
const connectWebSocket = () => {
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    devLog('嵌入式数字人切换为HTTP聊天模式')
    useHttpFallback = true
    return
  }

  const url = WS_URL_DIRECT
  devLog(`嵌入式数字人尝试连接 (第${reconnectAttempts + 1}次):`, url)
  
  try {
    ws = new WebSocket(url)
    
    ws.onopen = () => {
      devLog('嵌入式数字人 WebSocket 已连接:', url)
...
const handleMessage = (data: any) => {
  devLog('嵌入式数字人收到消息:', data.type, data.data)
  
  if (data.type === 'text_output' && !data.data.isPartial) {
    const text = data.data.text
    if (text) {
      pendingText = text
      devLog('嵌入式数字人文字已缓存，等待音频同步显示')
...
const playAudioWithText = async (blob: Blob) => {
  devLog('嵌入式数字人收到音频, size:', blob.size)
