...
const drawRoute = async () => {
  ...
  loading.value = true
  devLog(`开始绘制路线，景区数量: ${props.spots.length}`)
  devLog(`景区列表: ${props.spots.join(', ')}`)
  devLog(`传入的scenicData: ${props.scenicData?.length || 0} 条`)
  ...
  try {
    ...
    props.spots.forEach((spotId, index) => {
      ...
      if (coord) {
        ...
        devLog(`景区 ${index + 1}: ${name} [${coord[0]}, ${coord[1]}]`)
        
        // 添加标记
        addMarker(coord, index + 1, name, color)
      } else {
        console.warn(`未找到景区 ${spotId} 的坐标`)
      }
    })
...
const planRoute = async (coordinates: [number, number][]): Promise<[number, number][]> => {
  try {
    ...
    for (let i = 0; i < coordinates.length - 1; i++) {
      ...
      devLog(`规划路段 ${i + 1}: ${origin} -> ${destination}`)
      ...
      if (!response.ok) {
        console.warn(`路段 ${i + 1} 规划失败，使用直线连接`)
        continue
      }
      ...
      if (data.status === '1' && data.route?.paths?.length > 0) {
        ...
        if (steps && steps.length > 0) {
          hasValidPath = true
          devLog(`路段 ${i + 1} 规划成功，包含 ${steps.length} 个步骤`)
      ...
      if (!hasValidPath && apiEndpoint === 'transit') {
        console.warn(`路段 ${i + 1} 无公交路线，尝试使用驾车路线`)
        ...
        if (drivingResponse.ok) {
          ...
          if (drivingResult.code === 200) {
            ...
            if (drivingData.status === '1' && drivingData.route?.paths?.length > 0) {
              ...
              hasValidPath = true
              devLog(`路段 ${i + 1} 改用驾车路线，包含 ${steps.length} 个步骤`)
      ...
      if (!hasValidPath) {
        console.warn(`路段 ${i + 1} 无有效路径数据`)
      }
    }
    
    // 如果成功获取路径数据，返回；否则降级到直线
    if (routePath.length > 0) {
      devLog(`路径规划完成，共 ${routePath.length} 个坐标点`)
      return routePath
    } else {
      console.warn('所有路段规划失败，降级使用直线连接')
      return coordinates
    }
  } catch (error) {
