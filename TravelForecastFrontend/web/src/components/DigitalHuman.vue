...
watch(() => digitalHumanStore.pendingNarration, (prompt) => {
  ...
  devLog('[导览] 触发自动讲解:', narrationPrompt.substring(0, 40) + '...')
...
const connectWebSocket = () => {
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    ...
    devLog('WebSocket连接失败，切换为HTTP聊天模式（通过AI服务）')
    useHttpFallback = true
    return
  }
  // 直连数字人后端（有LLM+TTS缓存）
  const url = WS_URL_DIRECT
  devLog(`尝试连接 WebSocket (第${reconnectAttempts + 1}次):`, url)
  try {
    ws = new WebSocket(url)
    ws.onopen = () => { 
      devLog('数字人 WebSocket 已连接:', url)
      reconnectAttempts = 0 
...
const handleMessage = (data: any) => {
  devLog('收到消息:', data.type, data.data)
  ...
  if (data.type === 'viseme') {
    ...
    devLog('[Viseme] 收到口型数据:', data.data?.visemes?.length, '帧')
  }
}
...
const playAudioWithText = async (blob: Blob) => {
  devLog('收到音频，准备播放, size:', blob.size)
  ...
  if (!audioUnlocked) {
    devLog('[Audio] 浏览器未交互，缓存音频等待点击后播放')
