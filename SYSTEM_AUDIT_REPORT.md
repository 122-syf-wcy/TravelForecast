...
| 位置 | 问题 | 建议 |
|------|------|------|
| `TravelForecastBackend/application.yml` | 数据库密码 `<REDACTED_DB_PASSWORD>`、通义千问 API Key `sk-c444b733...`、高德 API Key、OSS AccessKey 明文 | 改为环境变量 `${DB_PASSWORD}`、`${DASHSCOPE_API_KEY}` 等 |
...
- **TravelForecastBackend** 的 `application-prod.yml` 使用 `travel_user` / `Travel@2024`，与主配置 `root` / `<REDACTED_DB_PASSWORD>` 不一致，需确认生产库用户。
