...
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), '挑战杯项目计划书_智教黔行.docx')
...
def main():
    ...
    add_para(doc, '本项目面向贵州六盘水"中国凉都"文旅产业数字化转型需求，设计并实现了"智教黔行·智慧文旅一体化平台"。平台采用微服务架构，融合AI大模型（通义千问）、深度学习客流预测（ARIMA+LSTM双流融合模型）、3D数字人导游、红色研学教育、文创电商等核心功能，覆盖"游前规划—游中导览—游后消费"全链路。')
